 ...
 * 
 * @param <S>
 *            the transportable class this object is backing.
 * 
...
public abstract class AbstractTransportableEntryListBacking<S extends Transportable>
	 ...
	 * (entryN) [after]), where (entry1) is the contents of the toString method
	 * for the entry.
	 * 
	 * @return a string created from appending the toString method in the
	 *         entries.
	 */
	@Override
	 ...
	 *             if a parse exception occurs before reading in the entries.
	 */
	protected void readBeforeEntries(final TransportReader reader)
			throws ParseException {
	}
	 ...
	 * 
	 * @throws ParseException
	 *             if a parse exception occurs after reading in the entries.
	 */
	protected void readAfterEntries(final TransportReader reader)
			throws ParseException {
	}
	 ...
	 * @throws ParseException
	 *             if a parse exception occurs when reading in the entries.
	 */
	@Override
	 ...
	 * 
	 * @param writer
	 *            the writer to write the data out to.
	 */
	@Override
	 ...
	 * Implementing classes should not write attributes without enclosing them
	 * in nodes.
	 * 
	 * @param writer
	...
	/**
	 * Callback that is invoked before the entry is removed from the backing
	 * list in the {@link #removeEntry(int)} call.
	 * 
	 * @param index
	 *            the index to remove.
	 */
	...
	/**
	 * Callback that is invoked after the entry is removed from the backing list
	 * in the {@link #removeEntry(int)} call.
	 * 
	 * @param index
	 *            the index to remove.
	 */
	...
	/**
	 * Callback that is invoked before the entry is added to the backing list in
	 * the {@link #removeEntry(int)} call.
	 * 
	 * @param entry
	 *            the entry to be added.
	 * 
	 * @throws IllegalStateException
	 *             if the method is modifying state
	 */
