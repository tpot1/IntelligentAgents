 ...
 */
public class AdNetProfitsPanel extends SimulationTabPanel {
	...
	public AdNetProfitsPanel(TACAASimulationPanel simulationPanel) {
	...
	protected void initialize() {
		...
		JFreeChart chart = createDaySeriesChartWithColors(null,
	...
	private void setAgent(int agent, int role, String name, int participantID) {
		...
		if (series[index] == null
				&& TACAdxConstants.AD_NETOWRK_ROLE_ID == roles[index]) {
	...
	protected class DayListener implements TickListener {
		...
		public void tick(long serverTime) {
			AdNetProfitsPanel.this.tick(serverTime);
		}

		@Override
		public void simulationTick(long serverTime, int simulationDate) {
			AdNetProfitsPanel.this.simulationTick(serverTime, simulationDate);
		}
	}
	...
	protected class BankStatusListener extends ViewAdaptor {
		...
		public void participant(int agent, int role, String name,
				int participantID) {
			AdNetProfitsPanel.this
					.participant(agent, role, name, participantID);
		}
	}
}