...
public class ViewerChartFactory {
	...
	public static JFreeChart createChart(XYDataset xydataset, String title,
		...
		JFreeChart jfreechart = ChartFactory.createXYLineChart(title, xLabel,
				yLabel, xydataset, PlotOrientation.VERTICAL, false, false,
				false);
	...
	public static JFreeChart createCapacityChart(XYDataset xydataset,
			String title, Color legendColor) {
		return createChart(xydataset, title, "Day", "% Capacity Used",
				legendColor);
	}
	...
	public static JFreeChart createDaySeriesChartWithColor(String s,
			XYDataset xydataset, Color legendColor) {
	...
	public static JFreeChart createDaySeriesChartWithColors(String s,
			XYDataset xydataset, boolean legend) {
	...
	public static JFreeChart createDifferenceChart(String title,
			XYDataset xydataset) {
		JFreeChart jfreechart = ChartFactory.createXYLineChart(title, "Day",
				"$", xydataset, PlotOrientation.VERTICAL, false, false, false);
		jfreechart.setBackgroundPaint(TACAAViewerConstants.CHART_BACKGROUND);
		...
		XYDifferenceRenderer renderer = new XYDifferenceRenderer(Color.green,
				Color.red, false);

		renderer.setSeriesPaint(0, ChartColor.DARK_GREEN);
		renderer.setSeriesPaint(1, ChartColor.DARK_RED);
		renderer.setBaseStroke(new BasicStroke(4f, BasicStroke.CAP_BUTT,
	...
	public static JFreeChart createDifferenceChart(String title,
			XYDataset xydataset, String bottomTitle, String sideTitle) {
		JFreeChart jfreechart = ChartFactory.createXYLineChart(title,
				bottomTitle, sideTitle, xydataset, PlotOrientation.VERTICAL,
				false, false, false);
		jfreechart.setBackgroundPaint(TACAAViewerConstants.CHART_BACKGROUND);
	...
	public static JFreeChart createAuctionChart(Query query, XYDataset xydataset) {
		JFreeChart jfreechart = ChartFactory.createXYLineChart(String.format(
				"Auction for (%s,%s)", query.getManufacturer(),
				query.getComponent()), "Day", "Bid [$]", xydataset,
	...
	public static JFreeChart createDaySeriesChartWithColors(String s,
			String yAxisLabel, XYDataset xydataset, boolean legend) {
		JFreeChart jfreechart = ChartFactory.createXYLineChart(s, "Day",
				yAxisLabel, xydataset, PlotOrientation.VERTICAL, true, false,
				false);
		jfreechart.setBackgroundPaint(TACAAViewerConstants.CHART_BACKGROUND);
		...
		if (legend) {
			...
			legendTitle
					.setBackgroundPaint(TACAAViewerConstants.CHART_BACKGROUND);
	...
	private static void formatRendererWithColor(XYPlot xyplot, Color legendColor) {
		...
		xyitemrenderer.setBaseStroke(new BasicStroke(4f, BasicStroke.CAP_BUTT,
				BasicStroke.JOIN_BEVEL));
	...
	private static void formatRendererWithColors(XYPlot xyplot) {
		...
		if (xyitemrenderer instanceof XYLineAndShapeRenderer) {
			...
			for (int i = 0; i < TACAAViewerConstants.LEGEND_COLORS.length; i++) {
				xylineandshaperenderer.setSeriesPaint(i,
						TACAAViewerConstants.LEGEND_COLORS[i]);
			}
		}
	}
}
